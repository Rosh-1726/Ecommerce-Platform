...
public class CartController {
     ...
     */
    @PostMapping("/add")
    @PreAuthorize("isAuthenticated()") //फक्त logged-in/authenticated user ही API वापरू शकतो.//म्हणजे SELLER, ADMIN, USER कोणताही authenticated user असेल तर API access करू शकतो.
    public ResponseEntity<CartResponse> addToCart( Authentication authentication, @RequestBody @Valid CartItemRequest request)
    {
...
/*
“First, the user sends a POST request with the product ID and quantity.
The API allows only authenticated users. I get the logged-in user's email
from the Authentication object. Then I pass the email, product ID and quantity
to the Cart Service. The service handles the business logic and
returns a CartResponse, which the controller returns with HTTP 200 OK.”
 */
